...
class SentimentModel(object):
	...
	learning_rate, lr_decay,batch_size, session,forward_only=False):
		...
		with tf.variable_scope("lstm") as scope:
			...
			pair_rep = tf.Variable(tf.zeros([array_ops.shape(nest.flatten(self.pairs)[0])[0], self.hidden_size, 1] , tf.float32))
			...
			def body(i): 
				...
				#temp = tf.SparseTensor(indices=[[0, 0,0], [0, 0,1]], values=[1, 2], dense_shape=[self.hidden_size, 1])
